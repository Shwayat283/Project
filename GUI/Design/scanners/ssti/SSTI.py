...
class SSTIScanner:
    ...
    def execute_shell_command(self, command):
        ...
        if not self.exploiter:
            # Try to create exploiter if vulnerability was found
            if not self.vulnerable_params:
                return "No vulnerability found. Cannot create shell."
            
            # Use the first vulnerable parameter
            vuln = self.vulnerable_params[0]
        ...
        try:
            ...
            return self.exploiter.execute_command(command)
        except Exception as e:
            return f"Error executing command: {str(e)}"

    def scan(self, url=None, url_list=None, threads=None, proxy=None, payload_list=None):
        """Run the SSTI scan"""
        if threads:
            self.threads = threads
        if proxy:
        ...
        self.attack_flow['phases'].append('Initialization')
        self.attack_flow['current_phase'] = 'Initialization'
    ...
    def generate_report(self, format="json", filename=None):
        ...
        if not filename or not self.vulnerable_params:
            return
            
        try:
            if format == "json":
                with open(filename, 'w') as f:
                    json.dump(self.vulnerable_params, f, indent=2)
            elif format == "csv":
                with open(filename, 'w', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    writer.writerow(["URL", "Parameter", "Engine", "Method", "Evidence"])
