...
base_url = input(f"{Fore.MAGENTA}{Style.BRIGHT}▶ Enter target URL :{Style.RESET_ALL}").strip()
...
print(Fore.MAGENTA+"{Style.BRIGHT}Request body parameters:"+Style.RESET_ALL)
...
ip_payloads = load_payloads(r"C:\Users\Admin\Desktop\مشروع التخرج\pathpayload.txt")
...
if input(f"{Fore.MAGENTA}{Style.BRIGHT}Do you have Burp Collaborator domain:{Style.RESET_ALL} ").strip().lower() == "yes":
    collab_domain =input(f"{Fore.MAGENTA}{Style.BRIGHT}Enter your Burp Collaborator domain :{Style.RESET_ALL} ").strip()
