...
import json
import csv
from datetime import datetime  # تم إصلاح نقص الاستيراد
...
def detect_os(response):
    """Detect OS based on response headers or body."""
    os_guess = "Unknown"
    ...
    if "internal server error" not in r.text.lower() and \
       ...
       "host must be" not in r.text.lower():
        ...
        print(Fore.GREEN + f"{Style.BRIGHT}The site is vulnerable to SSRF in {Fore.RED}{Style.BRIGHT}{url}{Fore.GREEN}{Style.BRIGHT} with this vulnerability :\n"
              f"{Fore.RED}{Style.BRIGHT}{payload} via {Fore.RED}{Style.BRIGHT}{param} parameter" + Style.RESET_ALL)
        return True
    return False
...
def path_payload(url, pay, param, payloads, proxies=None):
    ...
    if payloads in r.text.lower():
        ...
        results.append(result)
        print(Fore.GREEN + f"{Style.BRIGHT}The site is vulnerable to SSRF in {Fore.RED}{Style.BRIGHT}{url}{Fore.GREEN}{Style.BRIGHT} with this vulnerability :\n"
              f"{Fore.RED}{Style.BRIGHT}{pay} via {Fore.RED}{Style.BRIGHT}{param} parameter" + Style.RESET_ALL)
...
def process_payload(ip, i, target_url, payload, name, proxies=None):
    ...
    if ssrf_post(target_url, constructed_url + payload, name, return_bool=True, proxies=proxies):
        print(Fore.GREEN + f"{Style.BRIGHT}The site is vulnerable to SSRF in {Fore.RED}{Style.BRIGHT}{target_url}{Fore.GREEN}{Style.BRIGHT} with this With this vulnerability :\n"
              f"{Fore.RED}{Style.BRIGHT}{constructed_url + payload} via {Fore.RED}{Style.BRIGHT}{name} parameter" + Style.RESET_ALL)
