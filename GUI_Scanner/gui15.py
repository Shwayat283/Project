...
class ScannerApp(tk.Tk):
    ...
    def _create_settings_button(self):
        # إنشاء زر القائمة المنسدلة للإعدادات
        self.settings_btn = tk.Menubutton(self, text='⚙', font=('Segoe UI', 16), bg='#1a1a1a', fg='white', bd=0, relief='flat')
        ...
        # وضع الزر في الزاوية العلوية اليسرى
...
class LFIScannerWindow(tk.Toplevel):
    ...
    def _start_scan(self):
        urls = []
        # Determine input source: single URL or URL list
...
class ColorThemeWindow(tk.Toplevel):
    ...
    def _apply_theme(self, color):
        self.parent.style.configure('TFrame', background=color)
        self.parent.style.configure('TLabel', background=color, foreground='white')
        self.parent.style.configure('TButton', background='#ff9900', foreground='white')
        self.parent.style.map('TButton',
                              background=[('active', '#ffaa00'), ('pressed', '#ff8800')])
        self.parent.style.configure('TLabelframe', background=color, 
                                    relief='flat', borderwidth=5,
                                    foreground='#ff9900', font=('Segoe UI', 12, 'bold'))
        self.parent.style.configure('TLabelframe.Label', background=color, foreground='#ff9900')
